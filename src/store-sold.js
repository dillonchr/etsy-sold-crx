((numberOfSimultaneousRequests = 1) => {
    ...
    const cards = Array.from(document.querySelectorAll('a.buyer-card.card'));
    ...
    const queue = cards.map(elem => {
        ...
        const updateSoldLabel = price => elem
            .querySelector('.strong.text-gray > span')
            .insertAdjacentHTML('afterend', `<span style="color:red;margin-left:1em;">${price}</span>`);
